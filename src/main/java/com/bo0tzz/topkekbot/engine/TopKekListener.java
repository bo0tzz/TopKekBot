...
public class TopKekListener implements Listener {
    ...
    public TopKekListener(TelegramBot bot) {
        ...
        textActions = new LinkedList<TextAction>() {{
            ...
            add(new TextAction((t, ev) -> t.equalsIgnoreCase("fish go moo"), (e) -> {
                e.getMessage().getChat().sendMessage("TopKek_Bot notices that " + e.getMessage().getSender().getFullName() + " is truly enlightened.");
                return null;
            }));
